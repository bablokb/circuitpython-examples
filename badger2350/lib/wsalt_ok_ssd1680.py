...
# Waveshare
_START_SEQUENCE = (
  b"\x12\x80\x14"              # SWR soft reset and wait 20ms (DS: 10ms)
  b"\x74\x01\x54"              # Analog block control ??
  b"\x7e\x01\3b"               # digital block control ??
  b"\x01\x03\x07\x01\x00"      # DOC set display size x0107 = 264-1 lines
  b"\x11\x01\x03"              # DEM Ram data entry mode 0b011
  b"\x44\x02\x00\x15"          # SRX set RAM x-start (POR, x15 = 176/8-1 cols)
  b"\x45\x04\x00\x00\x07\x01"  # SRY set RAM y-start (0-263)
  b"\x3c\x01\x00"              # BWCTL border color (0: black, 5: gray/white?)
  b"\x2c\x01\x1c"              # VCOM voltage
  b"\x3f\x01\x22"              # EOPT
  b"\x03\x01\x17"              # VGH
  b"\x04\03\x41\x00\x32"       # VSH1, VSH2, VSL
  b"\x4e\x01\x00"              # SRXC ram x address
  b"\x4f\x02\x00\x00"          # SRYC ram y address (0)
)

# Waveshare
_REFRESH_SEQUENCE = (
  b"\x22\x01\xF7"              # DUC2  (waveshare: C7 does not work)
  b"\x20\x00"                  # ADUS
)

# DSM1: 1µA, DSM2: 0.7µA (DSM1 keeps RAM)
_STOP_SEQUENCE = b"\x10\x81\x01\x64"  # DSM Deep Sleep Mode 1
...
class SSD1680(EPaperDisplay):
  ...
  def __init__(
    self, bus: FourWire, speed=0, height=176, width=264, **kwargs
   ) -> None:
    self._speed = -1
    bus.reset()
